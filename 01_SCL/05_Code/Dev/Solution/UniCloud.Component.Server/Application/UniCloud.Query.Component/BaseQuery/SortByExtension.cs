...
namespace UniCloud.Query
{
	public static class SortByExtension
	{
        ...
        private static Expression<Func<TEntity, dynamic>> GetSortExpression<TEntity>(string sortPredicate) where TEntity : class
        {
            Type type = typeof(TEntity);
            ParameterExpression param = Expression.Parameter(type, "t");
            PropertyInfo property = type.GetProperty(sortPredicate);
            var sortExpression =
                Expression.Lambda<Func<TEntity, dynamic>>(Expression.Property(param, property),
