...
namespace BaseCodeTools.AutoGenerate.PropertyCode
{
   public class StringProperty:  BaseProperty
    {
       public StringProperty(string typeName)
           : base(typeName)
        {
       ...
       {
           sb.AppendLine(GeneratorHelper.GetTabString(3) + "Property(p => p." + p.Name + ").HasColumnType(\"nvarchar\").HasMaxLength(100);");
       }
       ...
       {
           sb.AppendLine(GeneratorHelper.GetTabString(3) + "Property(p => p." + p.Name + ").HasColumnName(\"" + GetOracleColumnName(p.Name) + "\").HasColumnType(\"nvarchar\");");
