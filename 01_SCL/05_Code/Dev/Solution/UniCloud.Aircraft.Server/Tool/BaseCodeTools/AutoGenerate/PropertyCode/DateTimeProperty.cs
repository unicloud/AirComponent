...
namespace BaseCodeTools.AutoGenerate.PropertyCode
{
   public class DateTimeProperty:  BaseProperty
    {
       public DateTimeProperty(string typeName)
           : base(typeName)
        {
       ...
       {
           sb.AppendLine(GeneratorHelper.GetTabString(3) + "Property(p => p." + p.Name + ").HasColumnType(\"datetime2\");");
       }
       ...
       {
           sb.AppendLine(GeneratorHelper.GetTabString(3) + "Property(p => p." + p.Name + ").HasColumnName(\"" + GetOracleColumnName(p.Name) + "\").HasColumnType(\"DateTime\");");
       }
    }
}
