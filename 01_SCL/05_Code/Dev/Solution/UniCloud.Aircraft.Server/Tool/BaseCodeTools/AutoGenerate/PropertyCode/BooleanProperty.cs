...
namespace BaseCodeTools.AutoGenerate.PropertyCode
{
    ...
    {
        public BooleanProperty(string typeName) : base(typeName)
        {
      ...
      public override void ConfigurationPropertySqlServer(PropertyInfo p, StringBuilder sb)
       {
           sb.AppendLine(GeneratorHelper.GetTabString(3) + "Property(p => p." + p.Name + ").HasColumnType(\"bit\");");           
       }
      ...
      {
          sb.AppendLine(GeneratorHelper.GetTabString(3) + "Property(p => p." + p.Name + ").HasColumnName(\"" + GetOracleColumnName(p.Name) + "\").HasColumnType(\"odp_internal_use_type\");");
      }
